function ma1_page_thru_trials_binoriv(runpath, list_successful_only, plot_trials, plot_2D, plot_summary, detect_saccades, detect_saccades_custom_settings)
...
for ii = 1:length(fieldname)
    ...
    for k = 1:length(trial)
        ...
        if plot_summary || plot_2D,
            ...
            idx_before_fix_hold = find(trial(k).state < statenum(ii));
            idx_during_fix_hold = find(trial(k).state == statenum(ii));
            idx_after_fix_hold = find(trial(k).state > statenum(ii));
            ...
            trial_fix_window(k, :) = [trial(k).eye.(fieldname{ii}).pos];
        ...
        if (list_successful_only == 1 && trial(k).success) || (list_successful_only == -1 && ~trial(k).success) || list_successful_only==0
            ...
            if plot_2D,
                ...
                w = nsidedpoly(100, ...
                    'Center', [trial(k).eye.(fieldname{ii}).x trial(k).eye.(fieldname{ii}).y], ...
                    'Radius', trial(k).eye.(fieldname{ii}).radius); plot(w, 'FaceColor', 'r');
                hold on;
